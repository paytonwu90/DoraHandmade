...
### 表單預覽欄位：用 `defaultValues` + `watch()`，取代 `useEffect` 衍生 state

購物車的購買人資訊預覽區塊，原本用 `buyerInfo` state 靠 `useEffect` 監聽 `watch()` 同步更新，導致每次輸入多觸發一次 render；改成用 `useForm` 的 `defaultValues` 設初始值、預覽區塊直接讀 `watch()` 的結果，移除這個多餘的 derived state，與購物車數量同步（`useCartItems.js`）採用同一個原則。
